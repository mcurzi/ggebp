...
# Construction of the biplot using 'ggplot2' functions:
  ...
  # Observations:
    ...
    # Outer polygon
    hull_idx <- chull(obsCoords$coordX, obsCoords$coordY)  # Convex hull
    hull_df  <- obsCoords[hull_idx, ]                      # Keeps only border points
    
    g <- g + geom_polygon(data = hull_df, aes(x = coordX, y = coordY), fill = NA, colour = "darkblue", linewidth = 0.5)
    
    # Radius lines (mega environments)
    n_points <- nrow(hull_df)
    p1_x     <- hull_df$coordX
    p1_y     <- hull_df$coordY
    p2_x     <- c(hull_df$coordX[-1], hull_df$coordX[1])
    p2_y     <- c(hull_df$coordY[-1], hull_df$coordY[1])
    
    # Calculate edge vectors (dx, dy) for each side of the polygon
    dx <- p2_x - p1_x
    dy <- p2_y - p1_y
    
    # Angle from perpendicular line to a vector (dx,dy) starting from origin (0,0)
    perp_angles <- atan2(dx, -dy) 
    
    # Radius lengths
    rmax     <- max(sqrt(hull_df$coordX^2 + hull_df$coordY^2)) * .9  # 0.9 is a factor to slightly reduce calculated lengths 
    lines_df <- data.frame(x = 0, y = 0, xend = rmax * cos(perp_angles), yend = rmax * sin(perp_angles))
    
    g <- g + geom_segment(data = lines_df, aes(x = x, y = y, xend = xend, yend = yend), linetype = "dashed", colour = "darkblue", linewidth = 0.8)
    ...
    g <- g + scale_fill_discrete(name="group") + scale_colour_discrete(guide="none") + coord_fixed(ratio = 1)

    if(sameLimits) {
      llim <- min(varCoords$coordX, obsCoords$coordX, varCoords$coordY, obsCoords$coordY, lines_df$xend, lines_df$yend)
      ulim <- max(varCoords$coordX, obsCoords$coordX, varCoords$coordY, obsCoords$coordY, lines_df$xend, lines_df$yend)
